    ...
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    //Tracking过程,都会用到mpORBextractorLeft作为特征点提取器
...
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp)
{
    ...
    return mCurrentFrame.mTcw.clone();      //返回当前坐标系到世界坐标系的转换矩阵Tcw
}
...
void Tracking::Track()
{
    ...
    else        // 步骤2：跟踪
    {
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(!mVelocity.empty())
                    {
                        bOKMM = TrackWithMotionModel();         //(1)运动模型跟踪
                        vpMPsMM = mCurrentFrame.mvpMapPoints;
                        vbOutMM = mCurrentFrame.mvbOutlier;
                        TcwMM = mCurrentFrame.mTcw.clone();
                    }
                    bOKReloc = Relocalization();                //(2)重定位
                    
                    //重定位没有成功,但运动模型跟踪的效果还可以.
                    if(bOKMM && !bOKReloc)
                    {
        ...
        // 在步骤2.1中主要是两两跟踪（恒速模型跟踪上一帧、跟踪参考关键帧帧），这里搜索局部关键帧后搜集所有局部MapPoints，
...
{
    if(mCurrentFrame.N>500)     //当前帧关键点数量足够多,才能进行初始化
    {
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    // 步骤2：通过特征点的BoW加快当前帧与参考关键帧帧之间的特征点匹配
...
bool Tracking::TrackWithMotionModel()
{
    ...
    // 在跟踪过程中，去除outlier的MapPoint，如果不及时增加,MapPoint会逐渐减少
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    // 步骤5：对于双目或RGBD摄像头，统计可以添加的MapPoints数量和不可添加的MapPoints数量.
...
/**
 * @brief 重定位，called by UpdateLocalMap()
 *
 * 重定位
 */
